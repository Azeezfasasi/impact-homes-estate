    ...
    <div className="max-w-7xl mx-auto p-0 md:p-6">
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Property Types</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-2">Manage property types available for properties</p>
          </div>
          <button
      ...
      {showAddForm && (
        <div className="mb-8 bg-white rounded-lg shadow-sm border p-4 sm:p-6">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4">Add New Property Type</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Property Type ID</label>
              <input
                type="text"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
          <div className="flex items-center gap-4">
            <div className="p-2 sm:p-3 bg-impact-gold/10 rounded-full flex-shrink-0">
            ...
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-gray-900">{propertyTypes.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-600">Default Types</p>
              <p className="text-xl sm:text-2xl font-bold text-gray-900">{propertyTypes.filter(pt => pt.isDefault).length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-600">Custom Types</p>
              <p className="text-xl sm:text-2xl font-bold text-gray-900">{propertyTypes.filter(pt => !pt.isDefault).length}</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="px-4 sm:px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Property Types</h2>
        </div>

        <div className="divide-y divide-gray-200">
          {propertyTypes.length === 0 ? (
            <div className="p-4 sm:p-6 text-center text-gray-500">
              <p className="text-sm sm:text-base">No property types found. Add a new property type to get started.</p>
            </div>
          ) : (
            propertyTypes.map((propertyType) => {
              ...
              return (
                <div key={propertyType.id} className="p-4 sm:p-6 hover:bg-gray-50">
                  {isEditing ? (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div className="flex items-start sm:items-center gap-3 sm:gap-4 flex-1 min-w-0">
                        <div className="text-2xl sm:text-4xl flex-shrink-0">{getIconLabel(propertyType.icon).split(' ')[0]}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-wrap">
                            <h3 className="text-base sm:text-lg font-medium text-gray-900">{propertyType.name}</h3>
                          ...
                          <p className="text-xs sm:text-sm text-gray-600">{propertyType.description}</p>
      ...
      <div className="mt-8 bg-white rounded-lg shadow-sm border p-4 sm:p-6">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4">All Property Types</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-3">Default Types</h3>
            <ul className="space-y-2">
              {propertyTypes
                .filter(pt => pt.isDefault)
                .map((propertyType) => (
                  <li key={propertyType.id} className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                    <span>{getIconLabel(propertyType.icon).split(' ')[0]}</span>
                    <span className="truncate">{propertyType.name}</span>
                  </li>
                ))}
            </ul>
          </div>
          ...
          <div>
            <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-3">Custom Types</h3>
            {propertyTypes.filter(pt => !pt.isDefault).length > 0 ? (
              ...
              <p className="text-xs sm:text-sm text-gray-500">No custom property types yet</p>
            )}
          </div>
        </div>
      </div>
